...
function generateSnakePath() {
    ...
    // Dynamic spacing configuration - increased by 200% to accommodate boxes
    const rowSpacing = 360; // Fixed spacing between rows (in viewBox units) - 200% increase from 120
    const marginX = 100;
    const marginY = 50;
...
function alignSnakeIcons() {
    ...
    icons.forEach((icon, index) => {
        ...
        try {
            ...
            // Position box centered above each icon
            const box = icon.querySelector('.snake-icon-box');
            if (box) {
                // Box is already centered with left: 50% and transform: translateX(-50%)
